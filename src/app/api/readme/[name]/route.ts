...
const fetchGitHubJson = async <T>(url: string): Promise<T | null> => {
  try {
    ...
    console.warn("GitHub README API request failed:", error);
    return null;
  }
};

const findStoredUser = async (name: string): Promise<{ id: string; name: string } | null> => {
  try {
    ...
    console.warn("Stored RPG user lookup failed:", error);
    return null;
  }
};

const fetchGitHubReadmeStats = async (name: string): Promise<{ profile: GitHubProfile | null; stats: GitHubReadmeStats }> => {
  const profile = await fetchGitHubJson<GitHubProfile>(
    `https://api.github.com/users/${encodeURIComponent(name)}`
  );
  const login = profile?.login || name;
  const encodedLogin = encodeURIComponent(login);

  const [commits, pullRequests, issues] = await Promise.all([
    fetchGitHubJson<GitHubSearchResult>(
      `https://api.github.com/search/commits?q=author%3A${encodedLogin}&per_page=1`
    ),
    fetchGitHubJson<GitHubSearchResult>(
      `https://api.github.com/search/issues?q=author%3A${encodedLogin}%20type%3Apr&per_page=1`
    ),
    fetchGitHubJson<GitHubSearchResult>(
      `https://api.github.com/search/issues?q=author%3A${encodedLogin}%20type%3Aissue&per_page=1`
    ),
  ]);
...
export async function GET(_request: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  try {
    const [user, githubData] = await Promise.all([
      findStoredUser(name),
      fetchGitHubReadmeStats(name),
    ]);
    const status = user ? await getUserBattleStatusById(user.id).catch(() => null) : null;
    const githubName = githubData.profile?.login || name;
    const displayName = githubData.profile?.name?.trim() || githubName;
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="680" height="300" viewBox="0 0 680 300" role="img" aria-label="${escapeXml(githubName)} Git-RPG guild status">
      <defs>
      ...
      <g filter="url(#shadow)">
        ...
        <text x="88" y="68" class="serif cream" font-size="24" font-weight="800">${escapeXml(displayName)}</text>
      ...
      <g filter="url(#shadow)">
        ...
        <text x="38" y="264" class="mono gold" font-size="11">${escapeXml(githubData.stats.commits)} COMMITS</text>
      ...
      <g filter="url(#shadow)">
        ...
        <text x="348" y="118" class="mono ink label">GITHUB ACTIVITY</text>
        <path d="M348 128h290" stroke="#6b4b26" stroke-opacity=".45"/>
        <text x="348" y="151" class="mono ink" font-size="10" font-weight="700">COMMITS</text>
        <text x="628" y="151" text-anchor="end" class="mono ink value">${escapeXml(githubData.stats.commits)}</text>
        <text x="348" y="181" class="mono ink" font-size="10" font-weight="700">PULL REQUESTS</text>
        <text x="628" y="181" text-anchor="end" class="mono ink value">${escapeXml(githubData.stats.pullRequests)}</text>
        <text x="348" y="211" class="mono ink" font-size="10" font-weight="700">ISSUES</text>
